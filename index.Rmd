...
As the process of this exploration went on, I sadly noticed that I couldn't perform all the statistical methods I would have wanted to on this data set, as it turned out to include a multitude of missing values in almost all columns, and performing na.omit() on the data resulted in only 200 or so observations remaining. Disillusioned about not being able (with my skill set) delve into Primary Component Analysis with regard to this data, I came into the conclusion of tackling the Linear regression approach instead.  I decided to choose the unnormalized data set instead of the normalized version, also available in the UCI repository, because the values in the data matrix seemed to be more easily comprehensible and comparable, whereas in the normalized data set all od the values were in the range of 0 to 1. What is more, the normalization performed on the data, I was afraid, would have left some extreme observations out. I learned that while performing linear regression, no standardization or normalization is essentially necessary, since the parameters and their signifigance is relatively the same whether performed on a standardized or an unstandardized data set.
...
```{r, fig.height=19, fig.width=20, message=FALSE, warning=FALSE, cache=TRUE}
library(GGally)
library(ggplot2)
library(gmodels)
...
```{r, message=FALSE, warning=FALSE, cache=TRUE}
p1 <- ggplot(CommunityCrime22, aes(x = racepctblack, y = ViolentCrimesPerPop))
...
p4 <- p3 + ggtitle("Percentage of Blacks versus Violent Crimes per capita")
p4
```

```{r, message=FALSE, warning=FALSE, cache=TRUE}
p11 <- ggplot(CommunityCrime22, aes(x = PctWorkMom, y = ViolentCrimesPerPop))
...
# add a main title and draw the plot
p44 <- p3 + ggtitle("Percentage of working moms vs Violent crimes per capita")
p44
```


```{r, message=FALSE, warning=FALSE, cache=TRUE}
p111 <- ggplot(CommunityCrime22, aes(x = PctPopUnderPov, y = ViolentCrimesPerPop))
...
# add a main title and draw the plot
p444 <- p3 + ggtitle("percentage of people under the poverty level versus Violent crimes per capita")
p444
